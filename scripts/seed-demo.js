...
const DEMO_EMAIL = 'christian.consuelo2+gymbro@gmail.com';
...
function calcTotalWeight(exerciseLogs) {
  return exerciseLogs.reduce((total, ex) => {
    return (
      total + ex.sets.filter((s) => s.completed).reduce((sum, s) => sum + s.weightLbs * s.reps, 0)
    );
  }, 0);
}
...
async function seed() {
  ...
  if (existing) {
    ...
    console.log(
      `✓ Cleared previous demo data (${sessions.deletedCount} sessions, ${routines.deletedCount} routines, ${plans.deletedCount} plans)`
    );
  }
  ...
  for (const sd of sessionsData) {
    ...
    const endTime = new Date(new Date(startTime).getTime() + sd.duration * 1000).toISOString();
